  ...
  public void transform(Map<SystemStreamPartition, MessageStream<IncomingSystemMessageEnvelope>> messageStreams) {
    ...
    messageStreams.values().forEach(entry -> {
        ...
        inputStream.filter(this::myFilter1)
          .window(Windows.sessionWindow(Time.milliseconds(100), sumAggregator)
            .setTriggers(new TriggersBuilder<>().withEarlyFiringsAfterCountAtleast(30000)
              .withLateFiringsAfterFirstMessage(Time.milliseconds(10))
              .build()));
