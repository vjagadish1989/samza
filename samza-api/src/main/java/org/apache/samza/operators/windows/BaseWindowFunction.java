...
/**
 *  The base class of all types of {@link WindowFunction}s. Sub-classes can specify the default triggering semantics
 *  for the {@link WindowFunction}, semantics for emission of early or late results and whether to accumulate or discard
 *  previous results.
 */
...
public class BaseWindowFunction<M extends MessageEnvelope, K, WV> implements WindowFunction<M, K, WindowKey<K>, WV, WindowOutput<WindowKey<K>, WV>> {
  ...
  /**
   * Defines the default triggering semantics for the {@link WindowFunction}.
   */
  ...
  public BaseWindowFunction(Function<M, K> keyExtractor, BiFunction<M, WV, WV> aggregator, Function<M, Long> eventTimeExtractor, List<TriggersBuilder.Trigger> defaultTriggers) {
  ...
  @Override
  public WindowFunction<M, K, WindowKey<K>, WV, WindowOutput<WindowKey<K>, WV>> setTriggers(TriggersBuilder.Triggers wndTrigger) {
