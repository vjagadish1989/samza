...
public final class Windows {
  ...
  /**
   * Returns a keyed {@link WindowFunction} that windows groups its values per-key into fixed-size processing time-based windows
   ...
   *  Windows.keyedTumblingWindow(keyExtractor, Time.seconds(10), maxAggregator));
   * }
   * </pre>
   *
  ...
  public static <M extends MessageEnvelope, K, WV> WindowFunction<M, K, WindowKey<K>, WV, WindowOutput<WindowKey<K>, WV>>
    ...
    return new BaseWindowFunction<M, K, WV>(keyFn, aggregateFunction, null, defaultTriggers);
  }
   ...
   *  Windows.keyedTumblingWindow(keyExtractor, Time.seconds(10)));
   * }
   * </pre>
   *
   * @param interval the duration in processing time on which the window is computed.
  ...
  /**
   * Returns a {@link WindowFunction} that windows values into fixed-size processing time-based windows and aggregates values
  ...
  public static <M extends MessageEnvelope, WV> WindowFunction<M, Void, WindowKey<Void>, WV, WindowOutput<WindowKey<Void>, WV>>
    ...
    return new BaseWindowFunction<M, Void, WV>(null, aggregateFunction, null, defaultTriggers);
  }
   ...
   *
   *  MessageStream<WindowOutput<WindowKey, Collection<Long>>> windowed = integerStream.window(Windows.tumblingWindow(Time.minutes(10)));
  ...
  public static <M extends MessageEnvelope> WindowFunction<M, Void, WindowKey<Void>, Collection<M>, WindowOutput<WindowKey<Void>, Collection<M>>> tumblingWindow(Time duration) {
    BiFunction<M, Collection<M>, Collection<M>> aggregator = (m, c) -> {
      c.add(m);
      return c;
    };
  ...
  /**
   * Returns a {@link WindowFunction} that windows values into sessions based on the {@code sessionGap} and aggregates values
  ...
  public static <M extends MessageEnvelope, WV> WindowFunction<M, Void, WindowKey<Void>, WV, WindowOutput<WindowKey<Void>, WV>> sessionWindow(Time sessionGap, BiFunction<M, WV, WV> aggregateFunction) {
  ...
  /**
   * Returns a {@link WindowFunction} that windows values into sessions based on the provided {@code sessionGap}. A Session
   ...
   * Function<Collection<Long>, Long>> percentile99 = ..
   *
   *  MessageStream<WindowOutput<WindowKey, Collection<Long>>> windowed = integerStream.window(Windows.sessionWindows(Time.minutes(10));
   *  MessageStream<Long> windowedPercentiles = windowed.map(windowedOutput -> percentile99(windowedOutput.getMessage())));
   * }
   * </pre>
   *
  ...
  public static <M extends MessageEnvelope> WindowFunction<M, Void, WindowKey<Void>, Collection<M>, WindowOutput<WindowKey<Void>, Collection<M>>> sessionWindow(Time sessionGap) {
    BiFunction<M, Collection<M>, Collection<M>> aggregator = (m, c) -> {
      c.add(m);
      return c;
    };
  ...
  /**
   * Returns a keyed {@link WindowFunction} that windows values into sessions based on the {@code sessionGap}
  ...
  public static <M extends MessageEnvelope, K, WV> WindowFunction<M, K, WindowKey<K>, WV, WindowOutput<WindowKey<K>, WV>> keyedSessionWindow(Function<M, K> keyFn, Time sessionGap, BiFunction<M, WV, WV> aggregateFunction) {
    List<TriggersBuilder.Trigger> defaultTriggers = new ArrayList<>();
  ...
  public static <M extends MessageEnvelope, K> WindowFunction<M, K, WindowKey<K>, Collection<M>, WindowOutput<WindowKey<K>, Collection<M>>> keyedSessionWindow(Function<M, K> keyFn, Time sessionGap) {
  ...
  /**
   * Returns a global {@link WindowFunction} that windows all values into a single large window. This window does not have a
   ...
   *   .withEarlyFiringsAfterFirstMessage(Time.minutes(10))
  ...
  /**
   * Returns a global {@link WindowFunction} that groups the values per-key and assigns them to their own global window.
   * This window does not have a default triggering behavior. The triggering behavior must be specified by setting early
   * triggers using the APIs of {@link TriggersBuilder}. Triggers are fired per-window per-key.
   *
  ...
  public static <M extends MessageEnvelope, K, WV> WindowFunction<M, K, WindowKey<K>, WV, WindowOutput<WindowKey<K>, WV>> keyedGlobalWindow(Function<M, K> keyFn, BiFunction<M, WV, WV> aggregateFunction) {
    return new BaseWindowFunction<M, K, WV>(keyFn, aggregateFunction, null, null);
  }
