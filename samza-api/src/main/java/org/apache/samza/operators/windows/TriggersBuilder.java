...
/**
 * Programmer API for specifying {@link Triggers} for a {@link WindowFunction}.
 *
 * <p> A {@link Triggers} instance specifies all the early and late triggers for a {@link WindowFunction}. It also specifies how
 ...
 *       .withEarlyFiringsAfterFirstMessage(Time.of(4, TimeUnit.SECONDS))
...
public final class TriggersBuilder<M extends MessageEnvelope, K, V> {
  ...
  /*
   * Defines a {@link Trigger} that triggers based on the specified time delay since the first {@link Message}
   * in the window pane.
   *
   */
  static class TimeSinceFirstMessageTrigger extends Trigger {
    ...
    TimeSinceFirstMessageTrigger(TriggerType type, long delayMillis, Time.TimeCharacteristic timeCharacteristic) {
      super(type);
      this.delayMillis = delayMillis;
      this.timeCharacteristic = timeCharacteristic;
    }
  ...
  static class Triggers {
    ...
    /**
     * Early and late firings, accumulation modes for a {@link WindowFunction}
     */
  ...
  /**
   * Add an early trigger that periodically emits results after a processing time delay from the first element in the pane.
   * Used to emit an early result before all data for the window have arrived.
   *
   * @param delay the delay period between firings
   * @return a reference to this object
   */
  public TriggersBuilder withEarlyFiringsAfterFirstMessage(Time delay) {
    earlyTriggers.add(new TimeSinceFirstMessageTrigger(TriggerType.EARLY, delay.toMilliseconds(), Time.TimeCharacteristic.PROCESSING_TIME));
    return this;
  }

  /**
   * Add a late trigger that periodically emits results after a processing time delay. Used to emit
   * results for late arrivals.
   *
   * @param delay the delay period between firings
   * @return a reference to this object
   */
  public TriggersBuilder withLateFiringsAfterFirstMessage(Time delay) {
    lateTriggers.add(new TimeSinceFirstMessageTrigger(TriggerType.LATE, delay.toMilliseconds(), Time.TimeCharacteristic.PROCESSING_TIME));
    return this;
  }

  /**
   * Add an early trigger that emits results after every {@code count} messages. Used to
   * emit an early result before all data for the window have arrived.
   *
  ...
  /**
   * Add a late trigger that after every {@code count} messages.  Used to emit
   * results for late arrivals.
   *
   * @param count the delay period between firings
   * @return a reference to this object
   */

  public TriggersBuilder withLateFiringsAfterCountAtleast(long count) {
    lateTriggers.add(new CountTrigger(TriggerType.LATE, count));
    return this;
  }
